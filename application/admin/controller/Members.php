...
class Members extends BaseMould
{
    ...
    public function index($pid=0)
    {
        ...
        if ($pid == 0)
        {
            ...
            if(empty($query))
            {
                $this->list = $this->m->where('parentid', $pid)->order($this->order)->paginate($this->site);
            }else{
                $this->list = $this->m->where('name|phone', 'like', "%{$query}%")->where('parentid', $pid)->order($this->order)->paginate($this->site);
            }
        }
        //处理下级数量
        foreach ($this->list as $k=>$val)
        {
            $this->list[$k]['num'] = $this->m->where('parentid', $val['id'])->count();

            $this->list[$k]['name'] = '<a href="'.url('admin/'.$this->mould->table.'s/index',['pid'=>$val['id']]).'">'.$this->list[$k]['name'].'('.$this->list[$k]['num'].')';
        }
    ...
    public function add()
    {
        ...
        //准备会员等级下拉内容
        $level = new Level;
        $levelarr = $level->order('level','asc')->select();
